...
依据：用户明确的“PLC 子设备维护配置、下发给 PLC 执行、优先 IoT 后对接 Hub”定位，以及 `需求+初步设计.md` 全文。Hub 源码仅作为后续对接背景。本文件将需求中的建议收敛为可实施的初版设计；下文保留当时的决策原文。

**实现现状（0.2.0）：** 独立 Agent 已落地契约、配置事务、只读 Modbus TCP、只读 OPC UA 订阅、MQTT 配置/遥测闭环、Outbox、心跳、多架构镜像与未签名 WBM 开发包。仍未完成 Hub 对接、PLC 真机验收、OPC UA 证书加密、南向 MQTT、PLC Bridge、写命令与 OTA。当前能力以仓库 README 和各阶段说明为准。
