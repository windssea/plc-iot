...
def config_ack(payload, result, gateway):
    ...
    known = {'SCHEMA_INVALID','INVALID_JSON','PAYLOAD_TOO_LARGE','GATEWAY_MISMATCH',
             'DUPLICATE_DEVICE_ID','DUPLICATE_POINT_ID','INVALID_ADDRESS','INVALID_STALE_INTERVAL',
             'POINT_LIMIT_EXCEEDED','VERSION_CONFLICT','STALE_VERSION','RESOURCE_LIMIT','STORAGE_ERROR',
             'APPLY_FAILED','UNKNOWN_CONFIG_VERSION','UNKNOWN_DEVICE','UNKNOWN_POINT','EXPIRED_DATA',
             'INVALID_TIMESTAMP','DUPLICATE_VALUE'}
    def error(code, path=''):
